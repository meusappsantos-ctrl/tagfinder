...
import { LogOut, Tv, Radio, Cpu, ArrowLeft, ArrowRight, Search, Plus, Save, MapPin, Loader2, Navigation, Edit, X, Globe, Trash2, Map as MapIcon, Crosshair, Server, ImageIcon, CheckCircle, ChevronRight, Hash, Database, Clock, Navigation2, Share2, FileDown, Layers, Locate, Activity, MapPinOff, ExternalLink } from 'lucide-react';
      ...
      <div className="bg-slate-800 w-full h-[90vh] sm:h-[85vh] sm:max-w-6xl sm:rounded-[2.5rem] overflow-hidden flex flex-col border-t sm:border border-slate-700 shadow-2xl">
        <div className="p-4 sm:p-6 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
          ...
          <div className="flex gap-2">
            <button onClick={centerOnUser} className="p-2 bg-blue-600 text-white rounded-xl shadow-lg active:scale-95 transition-all"><Locate className="w-6 h-6" /></button>
            <button onClick={onClose} className="p-2 bg-slate-700 rounded-xl text-slate-400 active:scale-95 transition-all"><X className="w-6 h-6" /></button>
          </div>
        </div>
...
const ItemDetail: React.FC<{ item: GroupItem; groupKey: string; config: any; user: User; onClose: () => void; onDelete: (id: string) => void; }> = ({ item, groupKey, config, user, onClose, onDelete }) => {
  ...
  const handleClearLocation = () => {
    if (confirm("Deseja remover a marcação deste item do mapa? As coordenadas serão excluídas.")) {
      setLocation(null);
    }
  };
              ...
              <div className="flex gap-2">
                 ...
                 {isEditing && (
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => setIsEditing(false)} className="p-2 bg-white/10 rounded-lg transition-all hover:bg-white/20"><X className="w-5 h-5" /></button>
                    </div>
                 )}
              </div>
          </div>
          <div className="p-4 sm:p-8 lg:p-12 space-y-6 sm:space-y-10 flex-1 overflow-y-auto pb-24 sm:pb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 flex flex-col gap-4 shadow-xl">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                         <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Marcação GPS</h4>
                      ...
                      {isEditing && location && (
                         <button onClick={handleClearLocation} className="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition-all shadow-sm" title="Excluir marcação do mapa">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {isEditing ? (
                            <button onClick={handleGetLocation} className="w-full py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all active:scale-95">{gettingLocation ? "Obtendo..." : "Atualizar Localização"}</button>
                        ) : location && (
                            <>
                                <button onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&destination=${location.lat},${location.lng}`, '_blank')} className="w-full py-3 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl flex items-center justify-center gap-2 transition-all hover:bg-emerald-700 shadow-md"><Navigation2 className="w-3 h-3" /> Rota</button>
                                <div className="grid grid-cols-2 gap-2">
                                  <button onClick={() => window.open(`https://earth.google.com/web/search/${location.lat},${location.lng}`, '_blank')} className="w-full py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl flex items-center justify-center gap-1 transition-all hover:bg-blue-700"><Globe className="w-3 h-3" /> Earth</button>
                                  <button onClick={downloadItemKML} className="w-full py-3 bg-slate-700 text-white text-[10px] font-black uppercase tracking-widest rounded-xl flex items-center justify-center gap-1 transition-all hover:bg-slate-600"><FileDown className="w-3 h-3" /> KML</button>
                                </div>
                            </>
                        )}
                    </div>
                ...
                <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 flex items-center gap-4 shadow-xl">
                    ...
                    <div><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Data de Registro</h4><p className="text-xs sm:text-sm text-slate-200 font-bold">{item.createdAt ? item.createdAt.toDate().toLocaleDateString('pt-BR') : "---"}</p></div>
                </div>
              </div>
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
                    {Object.entries(editData).filter(([k]) => isKeyVisible(k)).map(([key, value]) => (
                        <div key={key} className="bg-slate-800/40 p-4 sm:p-6 rounded-xl border border-slate-700/50 hover:border-slate-600 transition-colors">
                          ...
                          {isEditing ? (
                            <input type="text" value={String(value)} onChange={(e) => setEditData({ ...editData, [key]: e.target.value })} className="w-full bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm outline-none font-bold focus:border-blue-500 transition-colors shadow-inner" />
                          ) : (
              ...
              {isEditing && (
                <div className="grid grid-cols-2 gap-4 pt-6">
                    <button onClick={() => { if(confirm("Deseja realmente excluir este registro?")) onDelete(item.id) }} className="py-5 rounded-2xl bg-red-600/20 text-red-400 font-black uppercase tracking-widest text-[10px] flex justify-center items-center gap-2 hover:bg-red-600/40 transition-all border border-red-500/10">
                        <Trash2 className="w-4 h-4" /> Excluir Ativo
                    </button>
                    <button onClick={handleSave} disabled={isSaving} className={`py-5 rounded-2xl text-white font-black uppercase tracking-widest text-[10px] ${config.color} shadow-2xl flex justify-center items-center gap-3 transition-all active:scale-95 disabled:opacity-50`}>
      ...
      <div className="p-4 sm:p-5 flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
             <button onClick={(e) => onEdit(e, item)} className="p-2 bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-500/30 transition-all shadow-sm"><Edit className="w-3.5 h-3.5" /></button>
             <button onClick={(e) => onDelete(e, item.id)} className="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/30 transition-all shadow-sm"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        </div>
...
const GroupPage: React.FC<{ groupKey: GroupType; user: User; onBack: () => void; initialSelectedItem?: GroupItem | null }> = ({ groupKey, user, onBack, initialSelectedItem }) => {
  ...
  const filteredItems = items.filter(i => {
    ...
    // Corrigido: Agora a busca percorre todos os valores dentro do objeto 'data'
    // Isso garante que IP / Equipamento, Switches, e qualquer outro campo sejam filtrados corretamente
    const dataValues = Object.values(i.data || {}).map(v => String(v).toLowerCase());
    const contentMatch = i.content.toLowerCase().includes(s);
    const dataMatch = dataValues.some(val => val.includes(s));
    return contentMatch || dataMatch;
  });
  ...
  return (
    <div className="pb-24 sm:pb-12 animate-fadeIn">
      <div className="p-6 sm:p-10 mb-6 bg-slate-800/60 rounded-[2rem] sm:rounded-[3rem] border border-slate-700 flex flex-col gap-6 shadow-xl">
      ...
      <div className="flex flex-col gap-4 mb-8">
        <div className="relative">
          ...
          <input type="text" placeholder={`Buscar por Tag, IP, Local ou Equipamento...`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-slate-800 border border-slate-700 rounded-2xl text-white outline-none font-bold placeholder-slate-500 shadow-inner focus:border-blue-500 transition-colors" />
        </div>
      </div>
...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const handleSelectItemFromMap = (item: GroupItem) => {
    setItemFromMap(item);
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-5 sm:p-12 text-white relative overflow-hidden flex flex-col">
      ...
      {isMapModalOpen && <GlobalMapModal items={allMapItems} onClose={() => setIsMapModalOpen(false)} onSelectItem={handleSelectItemFromMap} />}
      
      <div className="max-w-6xl mx-auto w-full space-y-10 animate-fadeIn relative z-10 flex-1">
        <header className="flex flex-col gap-6">
          <div className="flex justify-between items-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/10 border border-blue-500/20 rounded-full text-[8px] font-black uppercase tracking-widest text-blue-400 shadow-sm">
              <div className="w-1 h-1 rounded-full bg-blue-400 animate-ping"></div> TagFinder Cloud
            </div>
            <button onClick={() => signOut(auth)} className="p-3 bg-slate-800 border border-slate-700 rounded-xl text-slate-500 active:scale-90 transition-all hover:bg-slate-700/50 shadow-md"><LogOut className="w-5 h-5" /></button>
          </div>
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs sm:text-sm font-medium">Gestão integrada e monitoramento georreferenciado.</p>
          </div>
          ...
          >
            ...
            <div className="flex items-center justify-between gap-4 relative z-10">
              <div className="flex items-center gap-6">
                ...
                <div>
                   <h3 className="text-xl sm:text-2xl font-black text-white tracking-tighter">Visualizar no Mapa Global</h3>
                   <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Gerencie marcações • GPS em tempo real</p>
                </div>
              </div>
      ...
      <footer className="py-8 text-center mt-auto">
        <p className="text-[8px] font-black text-slate-700 uppercase tracking-widest">&copy; 2024 TagFinder Enterprise • Gestão Técnica & Georreferenciamento</p>
      </footer>
    </div>
  );
};
