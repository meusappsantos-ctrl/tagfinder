...
// Fix: Import named members directly from firebase/firestore
import { 
  collection, 
  query, 
  orderBy, 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h2 className="text-2xl sm:text-4xl font-black text-white tracking-tighter leading-none">{groupKey === 'downloads' ? 'Arquivos & Links' : 'Inventário Ativo'}</h2>
            </div>
          </div>
      ...
      <div className="bg-slate-900 min-h-screen sm:min-h-[600px] sm:rounded-[3rem] border-x sm:border border-slate-800 shadow-2xl flex flex-col animate-fadeIn">
          <div className={`p-6 sm:p-10 bg-gradient-to-r ${config.gradient} text-white flex justify-between items-center sticky top-0 z-20 shadow-xl`}>
              ...
              <div className="flex gap-2">
                {!isEditing ? (
                  <div className="flex gap-2">
                    <button onClick={() => { if(confirm("Remover este registro permanentemente?")) { onDelete(item.id); onClose(); } }} className="p-3 bg-red-500/20 text-red-400 border border-red-500/30 rounded-xl hover:bg-red-500 transition-all active:scale-95"><Trash2 size={20} /></button>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-950 flex flex-col pb-10">
              <div className="p-6 sm:p-12 space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-slate-900/80 p-6 rounded-3xl border border-slate-800 flex items-center gap-5 shadow-2xl">
                          ...
                          <div>
                              ...
                              <p className="text-sm font-black uppercase mt-1 tracking-tight">{item.createdAt ? item.createdAt.toDate().toLocaleDateString('pt-BR') : "---"}</p>
                          </div>
                      </div>
                    </div>
                  ) : (
                        ...
                        {previewUrl && !isEditing ? (
                           <div className="space-y-4">
                              <div className="flex items-center justify-between px-2">
                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><Eye size={14} className="text-cyan-500" /> Pré-visualização Integrada</h4>
                                <span className="text-[8px] font-bold text-slate-700 bg-slate-900 px-2 py-1 rounded uppercase tracking-tighter">Google Document Viewer</span>
                              </div>
                           ...
                           <div className="p-10 text-center bg-slate-900/40 rounded-3xl border border-dashed border-slate-800">
                             <IdCard size={48} className="mx-auto text-slate-700 mb-4 opacity-20" />
                             <p className="text-slate-500 text-[10px] font-black uppercase">Visualização Indisponível para este Link</p>
                           </div>
                        )}
                    </div>
                  )}
                  ...
                  <div className="space-y-6">
                      <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-widest ml-2 flex items-center gap-2"><Database size={14} className="text-blue-500" /> {isDownload ? 'Metadados do Arquivo' : 'Ficha Técnica do Ativo'}</h4>
